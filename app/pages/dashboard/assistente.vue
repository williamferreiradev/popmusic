  ...
  <div class="h-screen flex bg-light-bg dark:bg-dark-bg text-light-text dark:text-offwhite overflow-hidden">
    <aside class="hidden lg:flex w-72 shrink-0 border-r border-light-border dark:border-dark-border bg-light-surface dark:bg-dark-surface flex-col">
      <div class="p-4 border-b border-light-border dark:border-dark-border">
        <button @click="newConversation" class="w-full flex items-center justify-center gap-2 bg-primary hover:bg-primary-hover text-white font-semibold rounded-lg px-4 py-2.5"><Plus class="w-4 h-4" /> Nova conversa</button>
      </div>
      <div class="flex-1 overflow-y-auto p-2 space-y-1">
        <button v-for="conversation in conversations" :key="conversation.id" @click="activeId = conversation.id" class="w-full text-left p-3 rounded-lg group flex items-center gap-3" :class="activeId === conversation.id ? 'bg-primary/10 text-primary dark:text-offwhite' : 'hover:bg-light-bg dark:hover:bg-dark-bg'">
    ...
    <section class="flex-1 flex flex-col min-w-0">
        ...
        <div class="max-w-3xl mx-auto space-y-6">
          <div v-if="!messages.length" class="min-h-[55vh] flex flex-col items-center justify-center text-center">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 mt-6 w-full max-w-xl">
              <button v-for="suggestion in suggestions" :key="suggestion" @click="input = suggestion" class="p-3 text-sm text-left rounded-xl border border-light-border dark:border-dark-border bg-light-surface dark:bg-dark-surface hover:border-primary/50">{{ suggestion }}</button>
            </div>
          </div>
      ...
      <footer class="p-4 border-t border-light-border dark:border-dark-border bg-light-surface dark:bg-dark-surface">
        <form class="max-w-3xl mx-auto flex items-end gap-2" @submit.prevent="sendMessage">
          <button type="button" @click="toggleRecording" class="p-3 rounded-xl border" :class="isRecording ? 'bg-red-500 text-white border-red-500 animate-pulse' : 'border-light-border dark:border-dark-border hover:border-primary opacity-70'" :title="speechSupported ? 'Falar' : 'Voz indisponível neste navegador'"><Square v-if="isRecording" class="w-5 h-5" /><Mic v-else class="w-5 h-5" /></button>
          <textarea v-model="input" @keydown.enter.exact.prevent="sendMessage" rows="1" placeholder="Ex.: Cadastre o aluno João, CPF..., nascimento..." class="flex-1 resize-none max-h-32 rounded-xl border border-light-border dark:border-dark-border bg-light-bg dark:bg-dark-bg px-4 py-3 text-sm focus:outline-none focus:border-primary" />
