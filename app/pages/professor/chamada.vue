  ...
  <div class="p-8 w-full flex flex-col gap-6 min-h-[calc(100vh-theme(spacing.16))] relative">
    ...
    <div v-if="filteredTurmasCards.length > 0" class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3.5">
      ...
      >
        ...
        <div class="flex items-center justify-between text-[11px] text-light-text/60 dark:text-offwhite/60">
          <span>{{ t.salas?.nome || 'Sala não definida' }}</span>
          <span>Capacidade: {{ t.capacidade_maxima }}</span>
        </div>
      </div>
    </div>
    ...
    <div v-if="currentState === 'error'" class="flex-1 flex flex-col items-center justify-center py-16 bg-red-500/10 border border-red-500/30 rounded-xl">
      <AlertTriangle class="w-10 h-10 mb-3 text-red-500" />
      <p class="font-bold text-red-600 dark:text-red-400">Não foi possível carregar a chamada.</p>
      <button type="button" class="mt-3 px-4 py-2 rounded-md bg-primary text-white text-xs font-bold" @click="executeSearch()">Tentar novamente</button>
    </div>
    ...
    <div v-if="currentState === 'loaded'" class="flex-1 bg-light-surface dark:bg-dark-surface border border-light-border dark:border-dark-border rounded-xl shadow-sm overflow-hidden flex flex-col">
      ...
      <div class="px-4 sm:px-6 py-4 border-b border-light-border dark:border-dark-border bg-light-bg/30 dark:bg-dark-bg/30 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 shrink-0">
        ...
        <button 
          ...
          class="px-4 py-2 rounded-md text-xs font-bold bg-green-500 hover:bg-green-600 text-white shadow-sm transition-colors cursor-pointer disabled:opacity-40 disabled:cursor-not-allowed"
        >
        ...
        <button
          ...
          :disabled="registeredCount !== students.length || isFinalizing"
          class="px-4 py-2 rounded-md text-xs font-bold bg-primary hover:bg-primary-hover text-white shadow-sm disabled:opacity-40 disabled:cursor-not-allowed"
        >
          {{ isFinalizing ? 'Finalizando...' : 'Finalizar chamada' }}
        </button>
        <span v-else class="px-4 py-2 rounded-md text-xs font-bold bg-green-500/10 text-green-600 border border-green-500/30">Chamada finalizada</span>
        </div>
      </div>
...
const markAttendance = async (student: StudentRecord, status: 'presente' | 'falta') => {
  ...
  try {
    ...
    alert('Não foi possível salvar a presença. Verifique sua conexão e tente novamente.')
  }
}
...
const finalizeAttendance = async () => {
  if (isFinalized.value || isFinalizing.value) return
  if (registeredCount.value !== students.value.length) {
    alert('Registre a situação de todos os alunos antes de finalizar.')
    return
  }
  if (!confirm('Finalizar esta chamada? Depois disso o professor não poderá alterá-la.')) return

  isFinalizing.value = true
  try {
    const { error } = await (supabase as any).from('chamadas_aula').insert({
      turma_id: selectedClass.value,
      data_aula: selectedDate.value,
      finalizada_por: user.value?.id
    })
    if (error) throw error
    isFinalized.value = true
  } catch (error: any) {
    console.error('Erro ao finalizar chamada:', error)
    alert(`Não foi possível finalizar a chamada. ${error.message || 'Tente novamente.'}`)
  } finally {
...
const handleJustifyConfirm = async (data: { reason: string, observation?: string, scheduleReplacement: boolean, replacement?: { date: string, time: string } | null }) => {
