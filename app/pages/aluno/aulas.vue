    ...
    <div v-else class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
      ...
      >
        <div class="flex justify-between items-start mb-3">
          <h3 class="font-bold text-light-text dark:text-offwhite">{{ aula.turma_nome || aula.nome || aula.modalidade || 'Aula de Música' }}</h3>
        ...
        <div class="space-y-2 text-sm text-light-text/70 dark:text-offwhite/70">
          <p class="flex items-center gap-2">
            <Guitar class="w-4 h-4" /> {{ aula.modalidade || aula.modalidade_nome || 'Música' }}
          </p>
          <p class="flex items-center gap-2">
            <User class="w-4 h-4" /> Professor: {{ aula.professor || aula.professor_nome || 'A definir' }}
          </p>
          <p class="flex items-center gap-2">
            <MapPin class="w-4 h-4" /> {{ aula.sala || aula.sala_nome || 'Sala de Aula' }}
          </p>
        </div>
      </div>
    </div>
