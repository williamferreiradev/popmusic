...
begin
  if public.meu_papel() <> 'gestao'::public.papel_usuario then
    raise exception 'Apenas a gestao pode excluir alunos';
  end if;
  select usuario_id into v_usuario_id from public.alunos where id=p_aluno_id for update;
  ...
  delete from public.recibos where cobranca_id in (select id from public.cobrancas where aluno_id=p_aluno_id);
  delete from public.fluxo_caixa where origem_id in (select id from public.cobrancas where aluno_id=p_aluno_id);
